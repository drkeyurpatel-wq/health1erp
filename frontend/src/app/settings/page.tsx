...
import { Settings, Hospital, Bell, Shield, Globe, Palette } from "lucide-react";
  ...
  return (
    <AppShell>
      ...
      <Tabs defaultValue="hospital">
        <TabsList>
          <TabsTrigger value="hospital"><Hospital className="h-4 w-4 mr-1" />Hospital</TabsTrigger>
          ...
          <TabsTrigger value="language"><Globe className="h-4 w-4 mr-1" />Language</TabsTrigger>
        </TabsList>
          ...
          <Card><CardHeader><CardTitle>Hospital Information</CardTitle></CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Hospital Name</label>
                  <Input value={hospitalName} onChange={e => setHospitalName(e.target.value)} /></div>
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Phone</label>
                  <Input value={hospitalPhone} onChange={e => setHospitalPhone(e.target.value)} /></div>
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Email</label>
                  <Input value={hospitalEmail} onChange={e => setHospitalEmail(e.target.value)} /></div>
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Address</label>
                  <Input value={hospitalAddress} onChange={e => setHospitalAddress(e.target.value)} /></div>
              </div>
            ...
            <CardContent>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div><label className="block text-sm font-medium text-gray-700 mb-1">First Name</label>
                  <Input defaultValue={user?.first_name} /></div>
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Last Name</label>
                  <Input defaultValue={user?.last_name} /></div>
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Email</label>
                  <Input defaultValue={user?.email} /></div>
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Phone</label>
                  <Input defaultValue={user?.phone || ""} /></div>
              </div>
            ...
            <CardContent>
              <div className="space-y-3">
                {[
                  ...
                  <button
                    key={lang.code}
                    onClick={() => { setLanguage(lang.code); localStorage.setItem("language", lang.code); }}
                    className={`w-full flex items-center justify-between p-4 border rounded-lg transition-colors ${language === lang.code ? "border-primary-500 bg-primary-50" : "hover:bg-gray-50"}`}
                  >
                    <div><p className="font-medium">{lang.label}</p><p className="text-sm text-gray-500">{lang.native}</p></div>
                    {language === lang.code && <Badge>Active</Badge>}
                  </button>
                ))}
              </div>
            </CardContent>
